...
minimal coordinator blacklist-handling request drain-to-control-thread command shell is locally green
...
- **Latest: WORKER H3 minimal coordinator blacklist-handling request drain-to-control-thread command
  shell is locally green — the coordinator can drain a pending future blacklist-handling request into a
  control-thread command shell without applying blacklist policy or persistence.**
  REVIEW/FIX of the previous minimal coordinator pending blacklist-handling request queue/drain shell
  found no proven defects against `STATUS.md`, ADR-0015, ADR-0013, ADR-0008, and the RT-safety / layering
  rules: the shell is coordinator-side, headless, and non-vacuous; derives pending requests only from a
  valid deferred outcome-handling receipt; preserves watchdog-timeout vs crash distinction through queue,
  pending, drain, and after-drain empty paths; leaves initial/empty and acknowledged/cleared paths
  ...
  drain-to-control-thread command shell:
  `BlacklistHandlingCommand`, `BlacklistHandlingCommandResult`, and
  `drainPendingBlacklistHandlingRequestToControlCommand()`. The coordinator self-check now proves
  initial/empty command-drain paths stay empty; valid watchdog-timeout and crash pending requests drain
  into distinct future control-thread commands; after-drain and acknowledged/cleared paths stay
  empty/no-record; no blacklist policy is applied; no blacklist state is persisted; and no scanner,
  ...
  **187/187**.
  **Next:** REVIEW/FIX H3 minimal coordinator blacklist-handling request drain-to-control-thread command shell —
  ...
  hosting / `Node` contract unchanged), and the rolling-baton rule. Confirm the command shell is
  coordinator-side, headless, and non-vacuous; drains commands only from a valid pending blacklist-handling
  request; preserves watchdog-timeout vs crash distinction through command, pending-consumed, after-drain,
  and acknowledged/cleared empty paths; keeps no-policy/no-persistence flags false; does not enforce
  ...
  a minimal coordinator deferred blacklist-handling command receipt/status shell for future control-thread
