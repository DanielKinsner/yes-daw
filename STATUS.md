...
**Now:** H12 Timeline Clip right-edge trim via real-shell drag is local-green. Prior H12 checkpoints are remote-green:
...
`28416653470`. Current work lets the real Timeline child `Component` detect a selected Clip's right edge,
drag it left to shorten the Clip, derive the trim end from the shared Timeline canvas geometry, route the
edit through `UiAppModel` + `ProjectUndoStack`, persist the shortened non-destructive Clip metadata back
into the `.yesdaw` bundle, rebuild playback from the edited Project, and extend `YesDawUiInputCheck` so
trim, Undo, and Redo are all proven through the shipped `MainComponent` while the imported Clip remains
audible. Local gates: `git diff --check`; VS DevShell
...
**Next (Codex - H12 end-to-end): commit/push this Timeline trim checkpoint and wait for remote CI, then
continue H12 step 5 with fade/gain/snap/locate/loop gestures through real Component input plus
undo/redo parity.**
