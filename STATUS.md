...
`YesDawThemeAuditCheck` so a scratch raw `drawInspector` geometry value fails the audit.
...
tokenized; MainComponent painted header/transport/master-meter geometry and painted piano-roll geometry
are also tokenized; MainComponent painted inspector-panel geometry is also tokenized; broad UI migration is
not complete.
...
the theme audit, such as painted mixer-panel geometry, before broad UI migration.
