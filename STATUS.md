...
built/reviewed/green; the minimal coordinator blacklist-handling request/status shell is built/reviewed/green; the
minimal coordinator pending blacklist-handling request queue/drain shell is locally green
...
- **Latest: WORKER H3 minimal coordinator pending blacklist-handling request queue/drain shell is locally
  green — the coordinator can queue, inspect, and drain a future blacklist-handling request derived from
  the most recent deferred outcome-handling receipt without applying blacklist policy or persistence.**
  REVIEW/FIX of the previous minimal coordinator blacklist-handling request/status shell found no proven
  defects against `STATUS.md`, ADR-0015, ADR-0013, ADR-0008, and the RT-safety / layering rules: the
  shell is coordinator-side, headless, and non-vacuous; derives only from a valid deferred
  outcome-handling receipt; preserves watchdog-timeout vs crash distinction; leaves initial/empty,
  no-action, unconsumed, policy-applied, persistence-claimed, missing-control, mismatched, and
  acknowledged/cleared paths empty/no-record; keeps no-policy/no-persistence flags false; does not
  enforce blacklist policy, persist/cache blacklist state, scan/load plugins, execute graph rewiring, or
  claim graph recompile execution; existing deferred graph-change acknowledge/clear behavior remains
  intact; `YesDawPluginHost` remains the only JUCE plugin-hosting owner; the coordinator/check target does
  ...
  blacklist-handling request queue/drain surface:
  `queueBlacklistHandlingRequestForDeferredOutcomeHandling()`, `pendingBlacklistHandlingRequest()`, and
  `drainPendingBlacklistHandlingRequest()`. The coordinator self-check now proves initial/empty queue and
  drain paths stay empty; valid watchdog-timeout and crash receipts queue, expose, drain, and return to
  empty while preserving the failure-kind distinction; acknowledged/cleared receipts do not requeue; no
  blacklist policy is applied; no blacklist state is persisted; and no scanner, plugin loading, graph
  rewiring, graph recompile execution, ADR edits, goldens, subjective checks, or `[[clang::nonblocking]]`
  ...
  **Next:** REVIEW/FIX H3 minimal coordinator pending blacklist-handling request queue/drain shell —
  ...
  hosting / `Node` contract unchanged), and the rolling-baton rule. Confirm the queue/drain shell is
  coordinator-side, headless, and non-vacuous; derives pending requests only from a valid deferred
  outcome-handling receipt; preserves watchdog-timeout vs crash distinction through queue, pending, drain,
  and after-drain empty paths; leaves initial/empty and acknowledged/cleared paths empty/no-record; keeps
  no-policy/no-persistence flags false; does not enforce blacklist policy, persist/cache blacklist state,
  scan/load plugins, execute graph rewiring, or claim graph recompile execution; keeps JUCE hosting
  confined to `YesDawPluginHost`; and leaves `YESDAW_BUILD_APPS=OFF` pure sanitizer configs unaffected.
  Fix only proven defects. If clean and green, continue in the SAME baton to the next small worker chunk:
  a minimal coordinator blacklist-handling request drain-to-control-thread command shell for future
  blacklist handling, still without applying/enforcing blacklist policy, persistence/cache, scanner,
