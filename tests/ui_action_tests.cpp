           ...
           "[ui][actions]")
{
    ...
    REQUIRE (context.selectedRecordingMonitoringPolicy == UiRecordingMonitoringPolicy::DirectInput);
    REQUIRE (context.recordingMonitoringCount == 1);
    REQUIRE (registry.dispatch (UiActionId::RecordingSetMonitoringPolicy, context).dispatched);
    REQUIRE (context.recordingMonitoringSelected);
    REQUIRE (context.selectedRecordingMonitoringPolicy == UiRecordingMonitoringPolicy::LatencyCompensated);
    REQUIRE (context.recordingMonitoringCount == 2);
    REQUIRE (registry.dispatch (UiActionId::RecordingSetMonitoringPolicy, context).dispatched);
    REQUIRE (context.recordingMonitoringSelected);
    REQUIRE (context.selectedRecordingMonitoringPolicy == UiRecordingMonitoringPolicy::Off);
    REQUIRE (context.recordingMonitoringCount == 3);
    context.recordingTrackAvailable = true;
    ...
    REQUIRE (registry.dispatch (UiActionId::TimelineAutomationAddBreakpoint, context).dispatched);
    REQUIRE (context.activePanel == UiPanel::Timeline);
    REQUIRE (context.canUndo);
    REQUIRE_FALSE (context.canRedo);
           ...
           "[ui][actions][timeline-edit]")
{
    ...
    REQUIRE (model.project().clips.size() == 1u);
    REQUIRE (model.project().clips[0].timelineLength == 4096);
           ...
           "[ui][actions][mixer]")
{
    ...
    REQUIRE (snapshot.tracks[0].fxSlots.size() == 1u);
...
// G1.1 (plan §4): the keymap is per Focus context, sound (no chord bound twice within a
// context or against Global), aliases resolve, and docs/keymap-v2.md is EXACTLY what the
// descriptors render (generated by YesDawKeymapDoc — the document cannot drift from the code).
TEST_CASE ("keymap v2: per-context lookup, no conflicts, aliases, generated document",
           "[ui][actions][keymap][keymap-v2]")
{
    UiActionRegistry registry;
    const Keymap& keymap = registry.keymap();

    const std::vector<Keymap::Conflict> conflicts = keymap.conflicts();
    for (const Keymap::Conflict& conflict : conflicts)
        INFO ("conflict: " << registry.descriptor (conflict.first)->stableId << " vs "
              << registry.descriptor (conflict.second)->stableId << " on '"
              << keymap.chordFor (conflict.first) << "'");
    REQUIRE (conflicts.empty());

    // Aliases: Ctrl+Y is Redo everywhere; Return is Enter.
    for (const UiFocusContext focus : { UiFocusContext::Global, UiFocusContext::Arrange,
                                        UiFocusContext::PianoRoll, UiFocusContext::Mixer })
    {
        REQUIRE (keymap.actionForChord ("Ctrl+Y", focus) == UiActionId::EditRedo);
        REQUIRE (keymap.actionForChord ("Ctrl+Shift+Z", focus) == UiActionId::EditRedo);
        REQUIRE (keymap.actionForChord ("Return", focus) == keymap.actionForChord ("Enter", focus));
        REQUIRE (keymap.actionForChord ("Space", focus) == UiActionId::TransportTogglePlayStop);
    }

    // Per-context law: a context's own binding wins, Global fills in, another context's never
    // fires. Proven on a scratch keymap with one chord bound in two contexts.
    {
        Keymap scratch;
        REQUIRE (scratch.rebind (UiActionId::TimelineClipGainIncrease, "Alt+Shift+F9") == KeymapRebindStatus::Ok);
        // The same chord on a Global action conflicts with every context.
        REQUIRE (scratch.rebind (UiActionId::TransportToggleMetronome, "Alt+Shift+F9") == KeymapRebindStatus::DuplicateChord);
    }

    // The generated document.
    const std::string rendered = keymap.renderMarkdown();
    REQUIRE (rendered.find ("## Global") != std::string::npos);
    std::ifstream file (std::string (YESDAW_SOURCE_DIR) + "/docs/keymap-v2.md", std::ios::binary);
    REQUIRE (file.good());
    std::stringstream buffer;
    buffer << file.rdbuf();
    std::string committed = buffer.str();
