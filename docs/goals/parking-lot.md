...
- **CI classifies a push by its HEAD commit's diff, so a docs-only commit on top of code skips the code's run (2026-09-05, G3.8 cp2).** `.github/workflows/ci.yml` "Classify diff (docs-only fast path)" — the G3.8 cp2 push (`3e56c7d` + the docs-only `5225b3a`) ran the fast path and NO build over cp2's code; the next code head covered it. Classify the push RANGE (`before`..`sha`) or every commit in it, not the head alone (ci.yml is do-not-touch for the agent — Dan's edit). Process rule meanwhile: push a code commit alone; a docs-only commit on top of unrun code is never pushed in the same push.

- **The Sampler has no velocity layers, round-robin, per-pad envelope or choke groups (2026-09-05, G3.9 cp1).** `SamplerNode.h` — one sample per pad, one ADSR per instrument, no hi-hat choke; Logic's Drum Machine Designer and Live's Drum Rack have all four. Layers and round-robin extend `SamplerPad` to a list per key (a v32 `sampler_layers` table); choke groups a per-pad integer. Owner: G5 / the instrument polish pass, after SS-4 shows the kit in use.

- **The Sampler pad has no root-key / gain control in the panel (2026-09-05, G3.9 cp2).** `MainComponent.cpp` `InstrumentPanelComponent::paintOverChildren` paints name + mode; the row's `rootKey` and `gain` are set only by the model (root = the pad's key at load). A pad inspector (root, gain, one-shot, the sample's name) on a cell's double-click. Owner: the instrument polish pass.

- **The drum-mode roll does not fold to the pads or jump to them (2026-09-05, G3.9 cp2).** `UiPianoRollSurface.h` `drumMode` names the keys; the key window stays where it was (the G3.7 finding: no scroll-to-clip either). Live folds to the pads; Logic's drum editor shows only the kit's rows. A fold toggle in the roll header over `viewKeyCount`, and a jump to the lowest pad when the roll's clip changes to a Sampler track. Owner: the roll polish pass at G3 close.
...
- **The FX editor's rows are raw ParamSpec names (2026-09-05, G4.1 cp2 rubric).** `MainComponent.cpp` `FxEditorComponent` hosts the lane's parameter rows as they were ("eq.band.freq 1000.0Hz", a page chooser for the EQ's 24). G4.2 cp1 (2026-09-08) replaces the EQ's raw names with numbered bands, named controls and Bands 1-2 / 3-4 / 5-6 pages beside its response display. Other kinds still await their G4.2 faces (including the compressor GR meter). Owner: G4.2.
